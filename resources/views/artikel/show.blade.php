...
@section('seo_title', $post->getSeoTitle())
@section('seo_description', $post->getSeoDescription())
...
<script type="application/ld+json">
{
  ...
  "description": {{ json_encode($post->getSeoDescription()) }},
  "url": "{{ route('artikel.show', $post->slug) }}",
  @if($post->getThumbnailUrl())
...
<div class="pt-28 pb-0 px-4 bg-brand-dark-gray">
    <div class="max-w-3xl mx-auto">
        <nav class="flex items-center gap-2 text-xs text-brand-text-gray mb-6">
            <a href="{{ route('home') }}" class="hover:text-brand-white transition-colors">Beranda</a>
            <span>/</span>
            <a href="{{ route('artikel.index') }}" class="hover:text-brand-white transition-colors">Artikel</a>
            <span>/</span>
            <span class="text-brand-silver truncate max-w-xs">{{ $post->title }}</span>
        </nav>
    </div>
</div>
...
<article class="py-8 px-4 bg-brand-dark-gray">
    <div class="max-w-3xl mx-auto">
        ...
        <header class="mb-8">
            ...
            <p class="text-brand-text-gray text-lg mb-6">{{ $post->excerpt }}</p>
